...
impl<'a, E: Ext, M: PolkaVmInstance<E::T>> Runtime<'a, E, M> {
    pub fn handle_interrupt(
        ...
        instance: &mut M,
        program_counter: &mut Option<polkavm::ProgramCounter>,
    ) -> Option<ExecResult> {
    ...
    fn call(
        ...
        let call_outcome = match call_type {
            ...
            CallType::DelegateCall => {
                ...
                self.ext
                    .delegate_call(weight, deposit_limit, callee, input_data)
            }
        };
...
pub mod env {
    ...
    fn gas_limit(&mut self, memory: &mut M) -> Result<u64, TrapReason> {
        ...
        Ok(<E::T as frame_system::Config>::BlockWeights::get()
            .max_block
            .ref_time())
    }
    ...
    fn deposit_event(
        ...
        self.charge_gas(RuntimeCosts::DepositEvent {
            num_topic,
            len: data_len,
        })?;
